...
class Trainer():
    ...
    def train(self):
        ...
        for batch, (lr, hr, _, idx_scale) in enumerate(self.loader_train):
            ...
            # deal with sr to match hr
            if self.scale==[2,3,4] and self.args.model=='edsrmulti_bicubic':
                re_sr = self.model(lr, scale,out_size)
                sr=re_sr
            elif self.scale==[2,3,4]:
                re_sr=self.model(lr,scale)
                sr=re_sr

            elif self.args.model.find('metashuffle')>=1 and not self.args.small_mask: # 试metashuffle时执行这一行
                # get the position mat, using the scale_int size
                scale_coord_map, mask = self.input_matrix_wpn_new(H, W, scale)  # get the position matrix
                ...
                ###############################################################################################
                re_sr = self.model(lr, scale, out_size, scale_coord_map)
                re_sr=torch.masked_select(re_sr,mask.to('cuda'))
                sr = re_sr.contiguous().view(N, C, outH, outW)

            elif self.args.model.find('metashuffle')>=1 and self.args.small_mask: # 使用small_mask时执行这一行
                ...
                sr = self.model(lr, scale_int,out_size,scale_coord_map)
                # sr=torch.masked_select(re_sr,mask.to('cuda'))
                # sr=sr.contiguous().view(N,C,outH,outW)
                # sr = F.interpolate(re_sr, size=[outH, outW], mode='bicubic')
        ...
        with torch.no_grad():
            for idx_scale, scale in enumerate(self.scale):
                ...
                for idx_img, (lr, hr, filename, _) in enumerate(self.loader_test):
                    ...
                    elif self.args.model.find('metashuffle') >= 1 and self.args.small_mask:
                        # Using the scale size output
                        scale_coord_map, mask = self.pos_mapping_and_mask(H, W, scale)
                        if self.args.n_GPUs > 1 and not self.args.cpu:
                            scale_coord_map = torch.cat([scale_coord_map] * self.args.n_GPUs, 0)
