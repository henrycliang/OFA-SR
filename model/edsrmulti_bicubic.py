...
def make_model(args, parent=False):
    if args.dilation:
        ...
        return MultiEDSR_bicubic(args, dilated.dilated_conv)
    else:
        return MultiEDSR_bicubic(args)

class MultiEDSR_bicubic(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(MultiEDSR_bicubic, self).__init__()
    ...
    def forward(self, x,scale_int,out_size):
        ...
        x=nn.PixelShuffle(scale_int)(x)
        # out=F.interpolate(x,size=out_size,mode='bicubic')
        out=self.out_conv(x)

        # The upsample layer directly from LR
        out=F.interpolate(out, size=out_size, mode='bicubic',align_corners=True)
