...
class Pos2Weight(nn.Module):
    def __init__(self,inC, kernel_size=3, outC=3):
        ...
        self.SuperLinear=nn.Sequential(SuperLinear(128,32))
...
class MultiRDN_MetaShuffle(nn.Module):
    def __init__(self, args,conv=common.default_conv):
        ...
        superconv = [SuperConv2d(in_channels=G0, out_channels=32 * 32,
                                 kernel_size=3, padding=1, bias=True)]
        self.tail = nn.Sequential(*superconv)
        self.out_conv = nn.Sequential(conv(int(G0/2), 3, kernel_size=3))
    ...
    def forward(self, x, scale,out_size,pos_mat_dict):
        ...
        num_channels=math.ceil(scale*10-8)
        x = self.sub_mean(x)
        f__1 = self.SFENet1(x)
        x = self.SFENet2(f__1)
        ...
        module = self.tail[0]
        x = module(x, num_channels * int(self.G0/2))

        x_group = self.repeat_x(x, scale_int)
        x_group=torch.masked_select(x_group,mask.to('cuda'))
        x_group=x_group.contiguous().view(x.size(0),int(self.G0/2)*num_channels,out_size[0],out_size[1])
        x_group=x_group.permute(0,2,3,1)
        x_group=x_group.contiguous().view(x_group.size(0),x_group.size(1),x_group.size(2),int(self.G0/2),num_channels)
