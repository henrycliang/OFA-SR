...
class MultiRDN_MetaShuffle(nn.Module):
    ...
    def forward(self, x, scale,out_size,pos_mat_dict):
        ...
        scale=out_size[1]/x.size(3)
        num_channels=math.ceil(scale**2)
        ...
        x += f__1
        # feature_map_time=time.time()
        # print('Feature map time:',feature_map_time-start_time)
        ###############################################################################################################
        ...
        x_group=x_group.contiguous().view(x_group.size(0),out_size[0],out_size[1],self.G0,num_channels)

        # position vector
        local_weight = self.P2W(pos_mat.view(pos_mat.size(1), -1), num_channels)
        # view local_weight to the target shape
        local_weight = local_weight.contiguous().view(out_size[0], out_size[1], -1, 1)  # 将10000view成[50,2,50,2]时可能存在问题，要跟组成10000时的顺序一致。
        ...
        # up_time=time.time()
        # print('Up_scale time:',up_time-start_time)
