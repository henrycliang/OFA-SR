...
def make_model(args, parent=False):
    return MultiRDN_MetaShuffle_n(args)
...
class Pos2Weight(nn.Module):
    ...
    def forward(self,x,scale_int):
        ...
        output=output[:,:scale_int**2]
        output_n=self.softmax(output)
        return output_n


class MultiRDN_MetaShuffle_n(nn.Module):
    def __init__(self, args,conv=common.default_conv):
        super(MultiRDN_MetaShuffle_n, self).__init__()
        r = args.scale[0]
        self.G0 = args.G0
        G0=args.G0
        kSize = args.RDNkSize
    ...
    def forward(self, x, scale,out_size,pos_mat):
        ...
        module = self.tail[0]
        x = module(x, scale_int ** 2 * self.G0)

        x_group = self.repeat_x(x, scale_int)
        x_group = x_group.view(x_group.size(0), x_group.size(1), self.G0, scale_int ** 2, x_group.size(-2),
                               x_group.size(-1))
        ...
        local_weight = local_weight.contiguous().view(x.size(2), scale_int, x.size(3), scale_int, -1,
