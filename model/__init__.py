...
class Model(nn.Module):
    ...
    def forward(self, x, scale,out_size,pos_mat):
        self.idx_scale = scale
        target = self.get_model()
        if hasattr(target, 'set_scale'):
            print('model has the attribute set_scale, now setting the scale')
            target.set_scale(scale)

        if self.self_ensemble and not self.training: # 这个self_ensemble是什么?
            if self.chop:
        ...
        #     return self.forward_chop(x,pos_mat)
            ...
            # print('Now training the model {}'.format(self.args.model))
            return self.model(x)
        elif self.args.model=='edsrmulti_bicubic':
            return self.model(x,scale,out_size)
        else:
            return self.model(x,scale,out_size,pos_mat)
