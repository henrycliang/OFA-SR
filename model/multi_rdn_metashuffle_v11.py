...
class MultiRDN_MetaShuffle(nn.Module):
    ...
    def forward(self, x, scale,out_size,pos_mat_dict):
        ...
        x_group,pos_mat=self.bilinear_torch(x,out_size[0],out_size[1],scale=scale)
        pos_mat=pos_mat.cuda()
        # print(pos_mat[0,:60,1])
        # print(pos_mat0[0,:60,1])
    ...
    def bilinear_torch(self,img, dstH, dstW, scale,add_scale=True):
        N, C, srcH, srcW = img.size()
        UpImg = torch.zeros((N, C, dstH, dstW)).cuda()
        pos_mat = torch.zeros(dstH, dstW, 2)
        pad = nn.ReplicationPad2d(padding=(0, 1, 0, 1)).cuda()
        img = pad(img)
        # print(img.size())
        # print(img[0, 0, :, :])
        for i in torch.arange(0, dstH):
            for j in torch.arange(0, dstW):
                srcx = torch.div(i.float(), scale)
                srcy = torch.div(j.float(), scale)
                ...
                u = srcx - x
                v = srcy - y
                pos_mat[i, j, :] = torch.tensor([u, v])

                UpImg[:, :, i, j] = (1 - u) * (1 - v) * img[:, :, x, y] + u * (1 - v) * img[:, :, x + 1, y] + \
